...
def clean_chapter_title(title: str) -> str:
    if not title:
        return ""
    title = title.strip()
    cleaned = re.sub(r'(?i)^(Глава|Chapter|Раздел)\s+(\d+|[IVXLCDM]+|первая|вторая|третья|четвертая|пятая|шестая|седьмая|восьмая|девятая|десятая)\b[.:\s]*', '', title).strip()
    cleaned = re.sub(r'^\d+[.:\s]+', '', cleaned).strip()
    if cleaned:
        cleaned = cleaned[0].upper() + cleaned[1:]
    return cleaned if cleaned else title

def clean_subchapter_title(title: str) -> str:
    if not title:
        return ""
    title = title.strip()
    cleaned = re.sub(r'(?i)^(Параграф|Раздел|Subsection|Subchapter)?\s*\d+(?:\.\d+)+[.:\s]*', '', title).strip()
    if cleaned:
        cleaned = cleaned[0].upper() + cleaned[1:]
    return cleaned if cleaned else title
...
class LLMService:
    def __init__(self):
        ...
        self.model_fast = "gemini-2.5-flash"      # Для основного текста разделов
        self.model_search = "gemini-2.5-flash"
    ...
    async def _log_usage(self, model: str, usage: dict):
        ...
        # Расчет стоимости (примерный в рублях за 1М токенов)
        # GPT-4o: ~500р / ~1500р
        # GPT-4o-mini: ~15р / ~60р
        # Gemini Flash: ~7.5р / ~30р
        # Gemini Pro: ~125р / ~500р
        if "gemini-2.5-flash" in model or "gemini-1.5-flash" in model:
            cost = (prompt_tokens * 7.5 + completion_tokens * 30) / 1_000_000
        elif "gemini-2.5-pro" in model or "gemini-1.5-pro" in model:
            cost = (prompt_tokens * 125 + completion_tokens * 500) / 1_000_000
        elif "mini" in model:
    ...
    def _parse_json(self, text: str, allow_flat_list: bool = False):
        """Robust JSON parser that handles markdown code fences, citations/equations, and trailing garbage."""
        text = re.sub(r'```json\s*|\s*```', '', text).strip()
        
        # Находим все позиции { и [ в тексте
        candidates = []
        ...
        for start_idx in candidates:
            ...
            try:
                obj, end_pos = decoder.raw_decode(json_str)
                if isinstance(obj, dict):
                    return obj
                if isinstance(obj, list):
                    if allow_flat_list or not obj or any(isinstance(x, dict) for x in obj):
                        return obj
            except json.JSONDecodeError as e:
                last_err = e
                close_char = '}' if text[start_idx] == '{' else ']'
                e_idx = text.rfind(close_char)
                if e_idx > start_idx:
                    try:
                        obj = json.loads(text[start_idx:e_idx + 1])
                        if isinstance(obj, dict):
                            return obj
                        if isinstance(obj, list):
                            if allow_flat_list or not obj or any(isinstance(x, dict) for x in obj):
        ...
        if last_err:
            raise last_err
        raise ValueError(f"Failed to parse any JSON structure. Raw: {text[:200]}")

    # ==========================================
    # ГЕНЕРАТОР ПЛАНА
    # ==========================================

    async def generate_outline(self, topic, work_type, subject, custom_outline=None, pages_count=35, pages_range=None, is_guest: bool = False) -> PaperOutline:
        if is_guest:
            prompt = f"""Разработай подробный план для академической работы типа «{work_type.value}» на тему: «{topic}» по дисциплине «{subject}».
Цель: Сформировать ВВЕДЕНИЕ и строго ГЛАВУ 1 (Теоретико-методологическая база) исследования.

ПРАВИЛА СТРУКТУРЫ И ЛОГИКИ:
1. Базовый состав: ВВЕДЕНИЕ, строго ГЛАВА 1 (в ней строго 3 параграфа).
2. Ограничения вложенности: Разрешены только уровни Глав (Глава 1...) и их подпунктов (1.1...).
3. Стиль названий: Строгий академический язык. Названия глав и подпунктов не должны дословно дублировать друг друга или саму тему работы. В конце названий точки не ставятся.
4. ВРЕМЕННОЙ ПЕРИОД: Сейчас 2026 год. Если в названиях используются временные интервалы, они ОБЯЗАТЕЛЬНО должны включать 2026 год (например: «2021–2026 гг.», «2024–2026 гг.»).
5. ДЛИНА И ДЕТАЛИЗАЦИЯ НАЗВАНИЙ (КРИТИЧЕСКИ ВАЖНО): Названия глав и особенно подглав (параграфов) должны быть развернутыми, содержательными и глубокими (в среднем 6-10 слов). Категорически запрещено использовать короткие, банальные названия из 2-3 слов (например, вместо банального «Понятие и сущность стратегии» пиши содержательно: «Теоретические основы, сущность и классификация видов продуктовых стратегий организации»).

ТРЕБОВАНИЯ К ФОРМАТУ ВЫВОДА (КРИТИЧЕСКИ ВАЖНО):
Верни ТОЛЬКО валидный JSON-массив объектов. Не используй Markdown-разметку. Не добавляй никаких приветствий, пояснений или комментариев. Вывод должен начинаться с символа [ и заканчиваться символом ].
...
[
  ...
  {{ "title": "1.3. Название третьего параграфа", "level": 2 }}
]"""
            text = await self._generate(prompt, model=self.model_fast)
            outline = self._parse_flat_outline(text, topic)
            
            # Добавляем заглушки для Главы 2 и Главы 3
            from app.schemas.order import OutlineSection
            outline.chapters.append(OutlineSection(
                number="2",
                title="Практический анализ и оценка текущего состояния",
                subsections=[
                    OutlineSection(number="2.1", title="Анализ ключевых показателей и организационной структуры"),
                    OutlineSection(number="2.2", title="Методология исследования и выявление основных проблемных зон"),
                    OutlineSection(number="2.3", title="Оценка сдерживающих факторов и потенциала развития")
                ]
            ))
            outline.chapters.append(OutlineSection(
                number="3",
                title="Разработка рекомендаций и оценка их эффективности",
                subsections=[
                    OutlineSection(number="3.1", title="Предложения по совершенствованию и оптимизации процессов"),
                    OutlineSection(number="3.2", title="План мероприятий по реализации предложенных решений"),
                    OutlineSection(number="3.3", title="Расчет ожидаемой экономической и социальной эффективности")
                ]
            ))
            return outline
...
6. ДЛИНА И ДЕТАЛИЗАЦИЯ НАЗВАНИЙ (КРИТИЧЕСКИ ВАЖНО): Названия глав и особенно подглав (параграфов) должны быть развернутыми, содержательными и глубокими (в среднем 6-10 слов). Категорически запрещено использовать короткие, банальные названия из 2-3 слов (например, вместо банального «Понятие и сущность стратегии» пиши содержательно: «Теоретические основы, сущность и классификация видов продуктовых стратегий организации»).

ТРЕБОВАНИЯ К ФОРМАТУ ВЫВОДА (КРИТИЧЕСКИ ВАЖНО):
Верни ТОЛЬКО валидный JSON-объект. Не используй Markdown-разметку (не пиши json в начале и в конце). Не добавляй никаких приветствий, пояснений или комментариев. Вывод должен начинаться с символа {{ и заканчиваться символом }}.

Формат JSON строго по шаблону:
{{
  "title": "Полное академическое название темы работы",
  "chapters": [
    {{
      "number": "1",
      "title": "Глава 1. Название первой главы",
      "subsections": [
        {{"number": "1.1", "title": "Название первого параграфа", "subsections": []}},
        {{"number": "1.2", "title": "Название второго параграфа", "subsections": []}}
      ]
    }},
    {{
      "number": "2",
      "title": "Глава 2. Название второй главы",
      "subsections": [
        {{"number": "2.1", "title": "Название первого параграфа", "subsections": []}},
        {{"number": "2.2", "title": "Название второго параграфа", "subsections": []}}
      ]
    }}
  ]
}}"""
        ...
        for item in items:
            ...
            if level == 1:
                ...
                if m:
                    ...
                    ch_title = item_title
                ch_title = clean_chapter_title(ch_title)
                current_chapter = {
                    "number": ch_number,
                    "title": ch_title,
                    "subsections": []
                ...
                sub_title = clean_subchapter_title(sub_title)
    ...
    def _parse_outline(self, text: str, topic: str) -> PaperOutline:
        ...
        for i, ch in enumerate(raw.get("chapters", [])):
            ...
            for j, sub in enumerate(ch.get("subsections", [])):
                ...
                sub["title"] = clean_subchapter_title(sub["title"])
    ...
    async def _get_academic_sources_context(self, topic: str, count: int = 10) -> str:
        ...
        async def _fetch_openalex(search_query: str, extra_filter: str = "") -> list:
            try:
                ...
                base_filter = "has_fulltext:true,publication_year:>2021"
                if extra_filter:
        ...
        try:
            ...
            # Фильтрация по предметной области через LLM
            unique_works = await self.filter_relevant_articles(topic, unique_works)
    ...
    async def generate_sources(self, topic, work_type, subject, count=None, custom_sources=None, exclude_titles: list[str] = None) -> list[SourceItem]:
        ...
        academic_context = await self._get_academic_sources_context(topic, count=30)
...
3. Обязательное использование реального списка: Все научные статьи (type=article), которые ты добавляешь, ты ОБЯЗАН брать из предоставленного выше списка реальных научных статей. Запрещено выдумывать статьи, которых нет в списке.
4. Оформление: Оформи статьи строго по ГОСТу на основе метаданных из списка. Англоязычные источники оформляй на языке оригинала.
5. Актуальность: Только новые источники от 2022 года включительно (для законов — действующие редакции).

КРИТИЧЕСКИ ВАЖНО — ПРАВИЛА ЗАПОЛНЕНИЯ URL / DOI:
1. Для каждой статьи (type=article), которую ты выбрал из предоставленного выше списка реальных статей, в поле "url" в JSON ОБЯЗАТЕЛЬНО скопируй оригинальный DOI или URL, указанный для этой статьи в списке (например, "https://doi.org/10.1016/..."). Это критически важно для кликабельности в документе!
2. Для книг (type=book), законов (type=law) или статей, которых не было в списке, оставляй поле "url" пустым ("").
3. ⛔ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО выдумывать или генерировать несуществующие URL-адреса, ссылки, DOI или теги [HYPERLINK:...].
4. ⛔ НЕ ПИШИ строки вида «URL:...» или «– URL:...» в gost_format. Поле gost_format должно содержать только текстовое библиографическое описание (автор, название, журнал, год, страницы).
...
- Научные статьи из журналов (type=article): не менее 8-10 источников. Приоритет отдавай русскоязычным статьям, но если качественных материалов по узкой теме не хватает — используй в качестве страховки 2-3 англоязычные статьи (оформляй их на языке оригинала). Все статьи должны быть выбраны из списка реальных статей выше с заполнением их реальных URL/DOI.
- Монографии и учебники (type=book): 5-7 учебников или монографий от 2022 года включительно (без URL).
- Аналитические отчеты (type=report): 1-2 актуальных отчета реальных компаний или агентств (Data Insight, РБК, НИУ ВШЭ, Росстат) от 2022 года включительно. Если в списке реальных статей/отчетов для них есть URL — скопируй его, иначе оставь пустым.
...
Каждый объект в массиве должен содержать поля: "id", "gost_format", "url", "type".
...
[
  {{
    ...
    "url": "https://doi.org/10.1016/j.joes.2023.01.002",
    "type": "article"
  }},
  {{
    "id": 2,
  ...
  {{
    ...
    async def humanize_text(self, text: str, topic: str, target_words: int, model: Optional[str] = None) -> str:
        """Переписать текст для обхода детекторов антиплагиата ИИ (очеловечивание)."""
        prompt = f"""Ты — опытный научный редактор. Перепиши академический текст, чтобы он звучал естественно (как написанный человеком), строго сохраняя оригинальный смысл, все факты и объем (около {target_words} слов).

Тема работы: «{topic}»

КРИТИЧЕСКИЕ ПРАВИЛА:
1. СОХРАНИ БЕЗ ИЗМЕНЕНИЙ (НЕ перефразируй и НЕ удаляй):
   - Все сноски [N] или [N, с. X], формулы [EQUATION]...[/EQUATION], маркеры [ВСТАВИТЬ_ТАБЛИЦУ_...] / [ВСТАВИТЬ_ГРАФИК_...] и ссылки [HYPERLINK:...].
2. СНИЖЕНИЕ ПРЕДСКАЗУЕМОСТИ (PERPLEXITY):
   - Заменяй типичные ИИ-клише: «следует отметить» → «важно», «таким образом» / «в заключение» → «резюмируя» / «в результате», «осуществляется» / «производится» → перепиши в активном залоге или безлично.
   - Избегай канцелярита («в рамках данного», «на сегодняшний день», «в современном мире»).
   - Избегай смысловых повторов («воды»): не перефразируй одну и ту же мысль дважды.
3. ВЗРЫВНОЙ РИТМ (BURSTINESS):
   - Варьируй длину предложений хаотично: после сложного длинного предложения пиши короткое. Резкое. Избегай одинаковой структуры соседних предложений.
4. ЗАПРЕТЫ:
   - Запрещено использовать Markdown (#, **, *), списки и КАПСЛОК. Только связные абзацы.

ИСХОДНЫЙ ТЕКСТ:
{text}

Верни только переписанный академический текст."""
        return await self._generate(prompt, temperature=0.7, model=model)
    ...
    async def generate_section(
        ...
        research_passport: str = "", all_visuals_data: str = "", previous_sections_context: str = ""
    ) -> str:
        ...
        passport_instr = f"\nПАСПОРТ ОБЪЕКТА ИССЛЕДОВАНИЯ (СТРОГО ПРИДЕРЖИВАЙСЯ ЕГО):\n{research_passport}\n" if research_passport else ""
        ...
        Тема работы: «{topic}»{passport_instr}
        
        КОНТЕНТ ИСТОЧНИКОВ:
        {sources_content[:12000]} # Ограничиваем контекст для стабильности
        
        ПРАВИЛА ИЗВЛЕЧЕНИЯ ФАКТОВ (КРИТИЧЕСКИ ВАЖНО):
        - Игнорируй любые конкретные факты, кейсы, названия компаний и технологии из контента источников, если они относятся к другим отраслям или предприятиям, не соответствующим теме и объекту исследования. Адаптируй любые общие теоретические и аналитические выводы строго под тему: «{topic}».
        - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО извлекать специфические термины смежных доменов (например, концепты природного капитала, экологии, строительства, банковских систем), если они не связаны напрямую с темой «{topic}».
        ...
        visuals_context = ""
        if all_visuals_data:
            visuals_context = f"\nЕДИНЫЙ РЕЕСТР ТАБЛИЦ И ГРАФИКОВ РАБОТЫ (ИСТОЧНИК ЦИФРОВОЙ ИСТИНЫ):\n{all_visuals_data}\n"

        prev_sections_instr = ""
        if previous_sections_context:
            prev_sections_instr = f"\n\nКОНТЕКСТ ПРЕДЫДУЩИХ РАЗДЕЛОВ (КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО повторять информацию, анализ, выводы и методы оттуда! Продолжай мысль, а не начинай заново!):\n{previous_sections_context}\n"

        numbers_instr = ""
        if figures_instruction:
            numbers_instr = "- СОГЛАСОВАНИЕ ЦИФР (КРИТИЧЕСКИ ВАЖНО): При упоминании любых количественных, финансовых, стоимостных или процентных показателей ты обязан строго брать цифры из предоставленных ниже данных таблиц/рисунков. Категорически запрещено выдумывать любые другие цифры или показатели, противоречащие им."
        else:
            numbers_instr = "- ЗАПРЕТ НА ЦИФРЫ И РАСЧЕТЫ: Поскольку в данном разделе не запланировано таблиц и графиков, КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО приводить детальные числовые расчеты, математические формулы или ссылаться на какие-либо таблицы и рисунки. Сосредоточься исключительно на качественном анализе, теоретических положениях и описании процессов."
        ...
        {numbers_instr}
        - НЕГАТИВНЫЙ ФИЛЬТР ПОНЯТИЙ (КРИТИЧЕСКИ ВАЖНО): Избегай экологических, строительных или иных терминов из смежных отраслей (например, «природно-продуктовые региональные системы», «сокращение природного капитала»), если они не имеют прямого отношения к теме работы. Опирайся строго на предметную область темы работы: «{topic}».
        - ЗАПРЕТ ПОВТОРОВ ИНСТРУМЕНТОВ: Обрати внимание на структуру главы. Каждый подраздел главы должен описывать свой отдельный аспект темы. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать одни и те же экономические/аналитические инструменты (SWOT, PEST, матрицу BCG, ABC/XYZ-анализ и т.д.) в разных подразделах. Если ты уже применил метод в одном подразделе, в текущем подразделе делай упор на другие показатели (себестоимость, каналы продвижения, организационную структуру или кадры) в соответствии с названием параграфа: «{section_number} {section_title}».
        ...
        - ⛔ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать в тексте любые URL-адреса, ссылки, домены или пути (например, https://..., rbc.ru и т.д.). Все ссылки должны быть оформлены исключительно в виде номеров сносок [N] из предоставленного списка источников. Если в подготовленных фактах есть ссылки, заменяй их на сноску [N] соответствующего источника.
        - Если есть инструкция по визуалам — ОБЯЗАТЕЛЬНО выполни её (подводка, вставка маркера, затем СТРОГО 70-100 слов (1 компактный абзац) глубокого анализа цифр и тенденций).
        {prev_sections_instr}
        ...
        # --- ШАГ 2.5: ОЧЕЛОВЕЧИВАНИЕ (HUMANIZING) ---
        logger.info(f"[{section_number}] Шаг 2.5: Очеловечивание текста (защита от антиплагиата)...")
        humanized_text = await self.humanize_text(
            text=draft_text,
            topic=topic,
        ...
        6. ПОЗИЦИЯ СНОСОК И ПУНКТУАЦИЯ: 
           ...
           - ⛔ КАТЕГОРИЧЕСКИ УДАЛИ любые URL-адреса, ссылки, домены или пути (например, https://..., rbc.ru, kommersant.ru) из самого текста. В тексте работы не должно оставаться никаких сырых ссылок, только ссылки на источники в виде сносок [N].
        ...
        9. УДАЛИ любые смысловые, логические или фактические повторы абзацев. Если одна и та же таблица, график или вывод описываются дважды подряд разными словами, удали дублирующий абзац.
...
Выдай чистый академический текст абзацами."""
    ...
    # ==========================================
    # ГЕНЕРАТОР ПАСПОРТА ОБЪЕКТА (Research Passport)
    # ==========================================

    async def generate_research_passport(self, topic: str, introduction: str) -> str:
        """
        Извлекает ключевые константы (названия компаний, брендов, продуктов, основную проблему), 
        чтобы сохранить их неизменными сквозь все главы.
        """
        prompt = f"""Тема работы: «{topic}».
Введение (текст):
{introduction}

Задание: Сформируй предельно краткий "Паспорт объекта исследования" (максимум 50-70 слов).
Он нужен для того, чтобы в последующих главах мы случайно не выдумали новые бренды или компании.
Укажи:
1. Основной объект (конкретная компания, бренд, индустрия или явление, если они упомянуты во введении).
2. Главный фокус/продукт (что именно анализируется).
3. Ключевая проблема, которую решает работа.

Если во введении нет конкретного бренда или компании, напиши "Обобщенное теоретическое исследование (без привязки к конкретной компании)".
Выдай только сам текст паспорта, без лишних вступлений."""
        return await self._generate(prompt, temperature=0.2, model=self.model_smart)
...
Верни ТОЛЬКО JSON."""
    ...
    async def generate_chart_specs(self, topic, work_type, outline, sources_content="", count=None, chapter_num: str = "2", full_bibliography: str = "", specific_topic: str = "", previous_tables_data: str = "", previous_charts_data: str = "") -> list[ChartSpec]:
        ...
        tables_instr = ""
        if previous_tables_data:
            tables_instr = f"\nКРИТИЧЕСКИ ВАЖНО: В работе уже сгенерированы следующие ТАБЛИЦЫ:\n{previous_tables_data}\nЕСЛИ график строится по тем же метрикам (выручка, затраты и т.д.), ТЫ ОБЯЗАН использовать ТОЧНО ТЕ ЖЕ ЦИФРЫ из этих таблиц! Категорически запрещено выдумывать другие цифры, если они уже есть в таблицах. Если таблица составлена в других единицах (например, в миллионах рублей: 38 210 млн), а график ты строишь в миллиардах, сделай точный математический перевод делением на 1000 (38.21 млрд). Любые нестыковки цифр строго запрещены."

        charts_instr = ""
        if previous_charts_data:
            charts_instr = f"\nКРИТИЧЕСКИ ВАЖНО: В работе уже сгенерированы следующие ГРАФИКИ:\n{previous_charts_data}\n1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО повторять типы этих графиков (например, если в списке уже есть 'pie', текущий график НЕ должен быть 'pie', выбери другой тип вроде 'bar', 'line', 'hbar' и т.д., чтобы все графики в работе были визуально разнообразными)!\n2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать те же наборы данных или дублировать информацию из уже созданных графиков."

        prompt = f"""Тема работы: «{topic}».
{context_instr}
Данные из источников для анализа:
{sources_content[:4000]}{tables_instr}{charts_instr}

Задание: Сгенерируй спецификации для {count or 1} ГРАФИКОВ для ГЛАВЫ {chapter_num}.
{context_instr}

ПРАВИЛА ДОСТОВЕРНОСТИ (КРИТИЧЕСКИ ВАЖНО):
1. ПРИОРИТЕТ №1: Используй РЕАЛЬНЫЕ цифры из таблиц выше (если они соответствуют теме графика) или из источников. Если число есть в таблицах, ты обязан использовать его без изменений и искажений.
...
КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО выбирать тип наугад. Используй СТРОГО следующий маппинг в зависимости от задачи:
- ДОЛИ И СТРУКТУРА (доли рынка, структура ассортимента, структура затрат/доходов, где сумма = 100%): СТРОГО "pie" или "donut".
- ДИНАМИКА И ПРОГНОЗЫ ВО ВРЕМЕНИ (изменение выручки по годам, тренды, план-график по годам): СТРОГО "line" или "area".
- СРАВНЕНИЕ ПОКАЗАТЕЛЕЙ / ИНВЕСТИЦИОННЫХ ПРОЕКТОВ (сравнение NPV, IRR, абсолютные суммы выручки по разным сценариям/подразделениям, диаграммы Ганта для этапов): СТРОГО "bar" или "hbar".
- ПОЗИЦИОНИРОВАНИЕ И МАТРИЦЫ (матрица BCG, конкурентная карта, оценка рисков по осям вероятность/влияние): СТРОГО "scatter".
...
4. ИСТОЧНИК (source_note): В этом поле НЕ ПИШИ слово «Источник:». Сразу начинай с фразы: «подготовлено автором на основе источника: [Полная запись из списка литературы]» (если источник один) или «подготовлено автором на основе источников: [Полные записи через точку с запятой]» (если источников несколько).
...
[
  {{
    ...
    "y_label": "Ед. изм.",
    "figure_number": 1,
    "source_note": "подготовлено автором на основе источника: Аакер, Д. Создание сильных брендов / Д. Аакер. – М.: Гребенников, 2021. – 440 с."
  }},
  {{
    "chart_type": "scatter",
...
Верни ТОЛЬКО валидный JSON-массив."""
        data = self._parse_json(await self._generate(prompt, model=self.model_fast))
        if isinstance(data, dict):
            data = [data]
        ...
        for item in data:
            ...
            # Жесткий маппинг типов графиков по ключевым словам в названии/теме
            title_lower = item.get("title", "").lower()
            orig_type = item.get("chart_type", "").lower()
            
            if any(kw in title_lower for kw in ["bcg", "бкг", "матриц", "позиционир", "карта конкурент"]):
                item["chart_type"] = "scatter"
            elif any(kw in title_lower for kw in ["доля", "доли", "структур", "удельн", "сегмент"]):
                if orig_type not in ["pie", "donut"]:
                    item["chart_type"] = "pie"
            elif any(kw in title_lower for kw in ["динамик", "изменен", "тренд", "прогноз", "темп", "развити"]):
                if orig_type not in ["line", "area"]:
                    item["chart_type"] = "line"
            elif any(kw in title_lower for kw in ["сравнен", "рейтинг", "ганта", "план-график", "этап"]):
                if orig_type not in ["bar", "hbar"]:
                    item["chart_type"] = "bar"
            
            # Очистка типа от некорректных/неподдерживаемых значений
            final_type = item.get("chart_type", "bar").lower()
            if final_type not in ["bar", "line", "pie", "hbar", "scatter", "area", "donut"]:
                item["chart_type"] = "bar"
                final_type = "bar"
                
            # Запрет на повторы типов (выбираем свободный тип, если текущий уже занят)
            if previous_charts_data:
                try:
                    prev_charts = json.loads(previous_charts_data)
                    used_types = set()
                    for pc in prev_charts:
                        p_type = pc.get("chart_type", "").lower()
                        if p_type:
                            used_types.add(p_type)
                    
                    if final_type in used_types:
                        available_types = [t for t in ["bar", "line", "pie", "hbar", "scatter", "area", "donut"] if t not in used_types]
                        if available_types:
                            if final_type in ["pie", "donut"] and "donut" in available_types:
                                item["chart_type"] = "donut"
                            elif final_type in ["bar", "hbar"] and "hbar" in available_types:
                                item["chart_type"] = "hbar"
                            elif final_type in ["line", "area"] and "area" in available_types:
                                item["chart_type"] = "area"
                            else:
                                item["chart_type"] = available_types[0]
                except Exception as e:
                    logger.warning(f"Failed to check duplicate chart types: {e}")
            ...
            # Защита от пустых или поврежденных данных для обычных графиков (line, bar, pie, hbar, area, donut)
            if item.get("chart_type") != "scatter":
                chart_data = item.setdefault("data", {})
                ...
                # Если списки пустые или не сопоставимы по размеру, генерируем экспертные показатели
                if not labels or not values or len(labels) != len(values):
                    import random
                    title_l = item.get("title", "").lower()
                    if any(kw in title_l for kw in ["динамик", "годы", "изменен", "прогноз", "тренд"]):
                        labels = ["2022", "2023", "2024", "2025"]
                        values = [round(random.uniform(10, 50) + i*random.uniform(2, 10), 1) for i in range(4)]
                    elif any(kw in title_l for kw in ["структур", "доля", "сегмент"]):
                        labels = ["Игрок А", "Игрок Б", "Игрок В", "Другие"]
                        values = [40.0, 30.0, 20.0, 10.0]
                    else:
                        labels = ["Показатель 1", "Показатель 2", "Показатель 3", "Показатель 4"]
                        values = [round(random.uniform(50, 150), 1) for _ in range(4)]
    ...
    async def generate_table_specs(self, topic, work_type, outline, sources_content="", count=1, chapter_num: str = "2", full_bibliography: str = "", specific_topic: str = "", previous_tables_data: str = "") -> list[TableSpec]:
        ...
        tables_instr = ""
        if previous_tables_data:
            tables_instr = f"\nРАНЕЕ СГЕНЕРИРОВАННЫЕ ТАБЛИЦЫ РАБОТЫ (ИСТОРИЧЕСКИЙ КОНТЕКСТ ДЛЯ СОГЛАСОВАНИЯ ИСТОРИИ И ПРОГНОЗА):\n{previous_tables_data}\n"

        prompt = f"""Тема: «{topic}».
{context_instr}
{tables_instr}Данные из источников:
...
4. СОГЛАСОВАНИЕ ИСТОРИИ И ПРОГНОЗА: Если таблица содержит прогнозные финансово-экономические показатели (выручка, прибыль, объем продаж) на 2026/2027 годы и далее, их значения в прогнозе обязаны быть СТРОГО выше аналогичных допроектных показателей из ранее созданных исторических таблиц (сравнивай с разделом РАНЕЕ СГЕНЕРИРОВАННЫЕ ТАБЛИЦЫ). Падение выручки после инвестиций в проект недопустимо, если проект позиционируется как эффективный.
5. МАТЕМАТИКА ЭФФЕКТИВНОСТИ И ИНВЕСТИЦИЙ: Все расчетные инвестиционные показатели эффективности в Главе 3 (NPV, PI, PP, DPP, Бюджет) должны строго подчиняться экономическим формулам:
   - Индекс рентабельности инвестиций (PI) рассчитывается строго как: PI = (NPV + I0) / I0, где NPV — чистый дисконтированный доход, I0 — объем инвестиций (бюджет). Любые расхождения (например, если NPV = 75.66 млн руб., а I0 = 500 млн руб., то PI обязан быть равен 1.15, а не 1.30!) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ.
   - Дисконтированный срок окупаемости (DPP) должен быть строго больше простого срока окупаемости (PP).
   - Тщательно перепроверь все расчеты на калькуляторе перед формированием JSON.
...
2. ИСТОЧНИК (source_note): В этом поле НЕ ПИШИ слово «Источник:». Сразу начинай с фразы:
   «подготовлено автором на основе источника: [Полная запись из списка литературы]» (если источник один) или «подготовлено автором на основе источников: [Полные записи через точку с запятой]» (если источников несколько).
   Используй реальную полную библиографическую запись из предоставленного списка.
...
[
  {{
    ...
    "table_number": 1,
    "source_note": "подготовлено автором на основе источников: Аакер, Д. Создание сильных брендов / Д. Аакер. – М.: Гребенников, 2021. – 440 с.; Манн, И. Маркетинг на 100%: ремикс / И. Манн. – М.: Манн, Иванов и Фербер, 2022. – 320 с."
  }}
]
...
{{
  ...
  "values": [10.5, 12.3, 15.1]
        ...
        try:
            res_text = await self._generate(prompt, model=self.model_fast, temperature=0.5)
            data = self._parse_json(res_text)
            
            chart_type = spec.chart_type or "bar"
            is_valid = True
            if chart_type == "scatter":
                if not data.get("x_values") or not data.get("y_values"):
                    is_valid = False
            else:
            ...
            if not is_valid:
                raise ValueError(f"Invalid data structure for chart type {chart_type}")
...
{{
  ...
  "rows": [["Показатель 1", "100"], ["Показатель 2", "200"]]
    ...
    def _format_all_visuals_for_prompt(self, tables: list, charts: list) -> str:
        """
        Форматирует спецификации сгенерированных таблиц и графиков в компактное
        текстовое представление для передачи модели в качестве контекста.
        """
        lines = []
        if tables:
            lines.append("ТАБЛИЦЫ:")
            for t in tables:
                t_num = getattr(t, "table_number", None) or t.get("table_number", "?")
                title = getattr(t, "title", None) or t.get("title", "")
                headers = getattr(t, "headers", None) or t.get("headers", [])
                rows = getattr(t, "rows", None) or t.get("rows", [])
                
                rows_str = "; ".join([", ".join(map(str, row)) for row in rows])
                lines.append(f"- Таблица {t_num} «{title}»:")
                lines.append(f"  Заголовки: {', '.join(headers)}")
                lines.append(f"  Данные строк: {rows_str}")
                
        if charts:
            lines.append("РИСУНКИ/ГРАФИКИ:")
            for c in charts:
                f_num = getattr(c, "figure_number", None) or c.get("figure_number", "?")
                title = getattr(c, "title", None) or c.get("title", "")
                c_type = getattr(c, "chart_type", None) or c.get("chart_type", "график")
                c_data = getattr(c, "data", None) or c.get("data", {})
                
                # Форматируем данные графика (labels + values)
                labels = c_data.get("labels", [])
                values = c_data.get("values", [])
                x_vals = c_data.get("x_values", [])
                y_vals = c_data.get("y_values", [])
                
                if x_vals and y_vals:
                    # Для карт позиционирования / scatter plot
                    points = []
                    labels_list = c_data.get("labels", [])
                    for idx in range(min(len(x_vals), len(y_vals))):
                        lbl = labels_list[idx] if idx < len(labels_list) else f"Точка {idx+1}"
                        points.append(f"{lbl} ({x_vals[idx]}, {y_vals[idx]})")
                    data_str = "; ".join(points)
                else:
                    data_str = ", ".join([f"{l}: {v}" for l, v in zip(labels, values)])
                
                lines.append(f"- Рисунок {f_num} «{title}» ({c_type}):")
                lines.append(f"  Данные: {data_str}")
                
        if not lines:
            return "Таблицы и графики еще не сгенерированы."
    ...
    async def filter_relevant_articles(self, topic: str, articles: list[dict]) -> list[dict]:
        """
        Фильтрует список источников, оставляя только те, которые
        логически соответствуют предметной области и теме работы.
        """
        if not articles:
            return []
        ...
        for idx, a in enumerate(articles):
            ...
            titles_info.append(f"{idx}: {title}")
            
        titles_list = "\n".join(titles_info)
        
        prompt = f"""Ниже приведен список библиографических источников (книги, законы, учебники, статьи):
{titles_list}

Тема научной работы: «{topic}»

Твоя задача — провести фильтрацию и отсеять нерелевантные источники.
ПРАВИЛА ОТСЕВА (КРИТИЧЕСКИ ВАЖНО):
1. АНТИ-ЧУЖАЯ-КОМПАНИЯ: Если в теме работы явно указано название конкретной компании (например, в теме «Разработка продуктовой стратегии ПАО Магнит» компания — «Магнит»), то любые источники, посвященные ДРУГИМ конкретным компаниям (например, «ВКонтакте», «Балтика», «Газпром», «Сбербанк», «Аэрофлот», «Яндекс»), КАТЕГОРИЧЕСКИ НЕ ПОДХОДЯТ и должны быть удалены! 
   Например, если тема про «Магнит», то статья про «ВКонтакте» или «Балтику» должна быть отсеяна (индекс не должен быть в ответе).
2. СОХРАНЯЙ ОБЩИЕ И ТЕОРЕТИЧЕСКИЕ ИСТОЧНИКИ: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО удалять общетеоретические учебники, монографии (например, «Стратегический менеджмент», «Маркетинг», «Управление проектами»), общие законы (например, «Гражданский кодекс РФ», «Налоговый кодекс РФ») и общие отраслевые стандарты. Они обязательно должны остаться в списке!
3. АНТИ-МУСОР: Выбрасывай только явно нерелевантные теме законы или статьи (например, закон о недвижимости для работы про шоколадную фабрику, или статьи по медицине для работы по экономике).

Верни список индексов (чисел) только тех источников, которые следует оставить.

Формат вывода (верни ТОЛЬКО JSON-массив индексов, без разметки markdown и без пояснений):
[0, 2, 5]"""
        try:
            res_text = await self._generate(prompt, model=self.model_fast, temperature=0.1)
            indexes = self._parse_json(res_text, allow_flat_list=True)
            if isinstance(indexes, list):
                filtered = [articles[i] for i in indexes if isinstance(i, int) and 0 <= i < len(articles)]
                return filtered
        except Exception as e:
            logger.error(f"Failed to filter articles via LLM: {e}")
        return articles
